...
async function providerPayload(provider, identity) {
  ...
  let headers = provider.keyless || provider.authMode === "anonymous"
    ? {}
...
export async function discoverProviderModels(
  ...
  if (cached) {
    ...
    const legacyMetadata = Object.fromEntries(
      modelEntries(payload, provider)
        .map((entry) => [entry.id, discoveredMetadata(entry.raw)])
        .filter(([, value]) => Object.keys(value).length > 0),
    );
    metadata = Object.fromEntries(
      discovered.map((id) => [id, { ...(legacyMetadata[id] || {}), ...(metadata[id] || {}) }])
        .filter(([, value]) => Object.keys(value).length > 0),
    );
