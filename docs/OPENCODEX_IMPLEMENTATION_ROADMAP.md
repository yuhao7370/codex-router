...
| Provider resilience | Model failover, provider cooldowns, usage accounting, and bounded retries are implemented for the supported routed paths. | `src/model-failover.mjs`, `src/provider-usage.mjs`, `test/model-failover.test.mjs`, `test/model-failover-router.test.mjs` |
