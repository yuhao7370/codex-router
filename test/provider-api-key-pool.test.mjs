...
test("oversized maps and symlink state fail closed without truncation", async () => {
  ...
  const targetDirectory = mkdtempSync(path.join(root, "link-target-"));
  const targetPath = path.join(targetDirectory, "valid.json");
  const targetContents = JSON.stringify({ version: 1, providers: {} });
  writeFileSync(targetPath, targetContents);
  assert.equal(readProviderApiKeyPoolState(targetPath).valid, true);
  const linkedPath = path.join(root, "linked.json");
  // A final-path junction needs no Windows symlink privilege. This exercises
  // rejection of a linked state path, not traversal through linked ancestors.
  symlinkSync(
    process.platform === "win32" ? targetDirectory : targetPath,
    linkedPath,
  ...
  await assert.rejects(
    upsertProviderApiKey("openrouter", metadata(credential("linked", "LINKED")), { filePath: linkedPath }),
    /invalid; refusing to overwrite/i,
  );
