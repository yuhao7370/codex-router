...
const { privateFileIsProtected, protectPrivateFile } = await import("../src/file-security.mjs");
...
function withLeafSymlink(target, link, context, operation) {
  try { symlinkSync(target, link, "file"); } catch (error) {
    if (process.platform === "win32" && error.code === "EPERM") {
      context.skip("Windows account lacks file-symlink privilege");
      return;
    }
    throw error;
  }
  try { return operation(); } finally { unlinkSync(link); }
}

// Exercise real link traversal without requiring Windows file-symlink privilege.
// A junction in the parent leaves a regular readable file at the linked path.
function withLinkedFile(target, link, operation) {
  if (process.platform !== "win32") {
    symlinkSync(target, link);
    try { return operation(); } finally { unlinkSync(link); }
  }
  const parent = path.dirname(link);
  const relocated = parent + "-linked-fixture";
  assert.ok(path.resolve(parent).startsWith(path.resolve(root) + path.sep));
  assert.ok(path.resolve(relocated).startsWith(path.resolve(root) + path.sep));
  renameSync(parent, relocated);
  const fixtureFile = path.join(relocated, path.basename(link));
  const relocatedTarget = target.startsWith(parent + path.sep)
    ? path.join(relocated, path.relative(parent, target)) : target;
  try {
    if (existsSync(relocatedTarget)) copyFileSync(relocatedTarget, fixtureFile);
    else writeFileSync(fixtureFile, "{}\n");
    protectPrivateFile(fixtureFile);
    symlinkSync(relocated, parent, "junction");
    try {
      assert.equal(statSync(link).isFile(), true);
      assert.equal(privateFileIsProtected(link), true);
      const before = readFileSync(fixtureFile);
      const result = operation();
      assert.deepEqual(readFileSync(fixtureFile), before);
      return result;
    } finally { rmdirSync(parent); }
  } finally {
    rmSync(fixtureFile, { force: true });
    renameSync(relocated, parent);
  }
}
...
test("migration rejects unknown legacy fields and confined paths", () => {
  ...
  withLinkedFile(path.join(root, "state", "target.json"), link, () => {
    assert.throws(
...
test("credential writes and resolution do not traverse symlinked paths", async (t) => {
  ...
  assert.equal(existsSync(path.join(outside, "store.json")), false);
  ...
  await t.test("resolution rejects a file symlink", (context) => {
    withLeafSymlink(external, providerPath, context, () => {
...
test("failed migration restores the exact original bytes", () => {
  ...
  withLinkedFile(outside, latestPath, () => {
    assert.throws(
...
test("credential references are router-plane- and provider-bound at resolution time", async (t) => {
  ...
  rmSync(providerPath);
  await t.test("reference resolution rejects a file symlink", (context) => {
    withLeafSymlink(external, providerPath, context, () => {
