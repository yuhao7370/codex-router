...
import { chmodSync, existsSync, mkdirSync, mkdtempSync, readFileSync, renameSync, rmSync, statSync, symlinkSync, utimesSync, writeFileSync as rawWriteFileSync } from "node:fs";
...
test("profile switching rejects symlinked login files before mutating the active profile", async () => {
  ...
  writeFileSync(path.join(primaryHome, "auth.json"), firstAuth, { mode: 0o600 });
  writeFileSync(chatGPTSubscriptionAccountAuthPath(first.id, { homesDir }), firstAuth, { mode: 0o600 });
  const secondContents = JSON.stringify({ tokens: { access_token: "second-token", account_id: "second" } });
  writeFileSync(secondAuth, secondContents, { mode: 0o600 });
  const options = { filePath, homesDir, primaryHome, switchPath, platform: "darwin", processList: "", refreshCatalog: false };
  await ensureChatGPTProfileAccounts(options);
  assert.equal(readChatGPTProfileSwitchState(switchPath).active, first.id);
  const accountAuth = chatGPTSubscriptionAccountAuthPath(second.id, { homesDir });
  if (process.platform === "win32") {
    // A junction needs no file-symlink privilege. Keep a real auth file behind
    // the linked account directory so rejection must enforce link ancestry,
    // rather than merely reject a directory where a file was expected.
    const accountHome = path.dirname(accountAuth);
    const linkedHome = path.join(root, "linked-login");
    assert.ok(path.resolve(accountHome).startsWith(path.resolve(root) + path.sep));
    assert.ok(path.resolve(linkedHome).startsWith(path.resolve(root) + path.sep));
    renameSync(accountHome, linkedHome);
    writeFileSync(path.join(linkedHome, "auth.json"), secondContents, { mode: 0o600 });
    symlinkSync(linkedHome, accountHome, "junction");
  } else {
    symlinkSync(secondAuth, accountAuth, "file");
  }
  await assert.rejects(
    requestChatGPTProfileSwitch(second.id, options),
    /unavailable|symbolic-link/i,
  );
  assert.equal(readFileSync(path.join(primaryHome, "auth.json"), "utf8"), firstAuth);
  assert.equal(readChatGPTProfileSwitchState(switchPath).active, first.id);
  assert.equal(readFileSync(secondAuth, "utf8"), secondContents);
  if (process.platform === "win32") {
    assert.equal(readFileSync(path.join(root, "linked-login", "auth.json"), "utf8"), secondContents);
  }
});
