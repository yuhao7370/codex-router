...
test("automatic selection-only setup exposes only configured providers", () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(output).providers, ["deepseek", "lmstudio", "local", "local-router"]);
    const selection = JSON.parse(readFileSync(path.join(stateDir, "enabled-providers.json"), "utf8"));
    assert.deepEqual(selection.providers, ["deepseek", "lmstudio", "local", "local-router"]);
  } finally {
